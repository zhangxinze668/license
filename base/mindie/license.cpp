...
#ifndef LICENSE_TAG
#define LICENSE_TAG "MINDIE-130B"
#endif
...
#define INOUT_BUF_LENGTH 1024
    ...
    std::string board_name_path = (std::string)AY_OBFUSCATE("/sys/class/dmi/id/board_name");
    std::string board_name;
    if (fileExists(board_name_path)) {
        board_name = commandOutput("cat " + board_name_path);
    }

    std::string board_serial_path = (std::string)AY_OBFUSCATE("/sys/class/dmi/id/board_serial");
    std::string board_serial;
    if (fileExists(board_serial_path)) {
        board_serial = commandOutput("cat " + board_serial_path);
    }

    std::string board_vendor_path = (std::string)AY_OBFUSCATE("/sys/class/dmi/id/board_vendor");
    std::string board_vendor;
    if (fileExists(board_vendor_path)) {
        board_vendor = commandOutput("cat " + board_vendor_path);
    }

    std::string product_name_path = (std::string)AY_OBFUSCATE("/sys/class/dmi/id/product_name");
    std::string product_name;
    if (fileExists(product_name_path)) {
        product_name = commandOutput("cat " + product_name_path);
    }

    std::string product_version_path = (std::string)AY_OBFUSCATE("/sys/class/dmi/id/product_version");
    std::string product_version;
    if (fileExists(product_version_path)) {
        product_version = commandOutput("cat " + product_version_path);
    }

    std::string product_serial_path = (std::string)AY_OBFUSCATE("/sys/class/dmi/id/product_serial");
    std::string product_serial;
    if (fileExists(product_serial_path)) {
        product_serial = commandOutput("cat " + product_serial_path);
    }

    std::string product_uuid_path = (std::string)AY_OBFUSCATE("/sys/class/dmi/id/product_uuid");
    ...
    std::snprintf(InOutBuf.data(), InOutBuf.size(),
                  AY_OBFUSCATE("board_name:%s,board_serial:%s,board_vendor:%s,product_name:%s,product_version:%s,"
...
__attribute__((always_inline)) inline RSA *loadPublicKey() {
    std::string publicKeyPem =
        (std::string)AY_OBFUSCATE("-----BEGIN PUBLIC KEY-----\n"
    ...
    if (bio == nullptr) {
        throw std::runtime_error((std::string)AY_OBFUSCATE("fail to create bio for publicKey"));
    }

    RSA *rsa_public_key = PEM_read_bio_RSA_PUBKEY(bio, nullptr, nullptr, nullptr);
    if (rsa_public_key == nullptr) {
        BIO_free_all(bio);
        throw std::runtime_error((std::string)AY_OBFUSCATE("fail to load publicKey"));
    }
...
rsaVerify(const std::string &data, const std::vector<unsigned char> &signature, RSA *public_key) {
    ...
    if (!SHA256((unsigned char *)data.c_str(), data.size(), hash)) {
        throw std::runtime_error((std::string)AY_OBFUSCATE("fail to compute SHA-256 hash"));
    }

    // Verify the signature
    if (RSA_verify(NID_sha256, hash, SHA256_DIGEST_LENGTH, signature.data(), signature.size(), public_key) != 1) {
        char *err = ERR_error_string(ERR_get_error(), NULL);
        std::cout << (std::string)AY_OBFUSCATE("OpenSSL Error: ") << err << std::endl;
        return false;
    }
    return true;
}
...
__attribute__((always_inline)) inline std::string aesDecrypt(const std::vector<unsigned char> &ciphertext,
    ...
    if (!ctx) {
        throw std::runtime_error((std::string)AY_OBFUSCATE("fail to create EVP_CIPHER_CTX"));
    }

    if (EVP_DecryptInit_ex(ctx, EVP_aes_256_cbc(), NULL, key.data(), iv.data()) != 1) {
        throw std::runtime_error((std::string)AY_OBFUSCATE("fail to initialize decryption"));
    }
    ...
    if (EVP_DecryptUpdate(ctx, plaintext.data(), &len, ciphertext.data(), ciphertext.size()) != 1) {
        throw std::runtime_error((std::string)AY_OBFUSCATE("fail to decrypt data"));
    }
...
__attribute__((always_inline)) inline void check_license(const std::filesystem::path &path) {
    std::ifstream license_file(std::filesystem::path(path) / (std::string)AY_OBFUSCATE("license.txt"));
    if (!license_file.is_open()) {
        throw std::runtime_error((std::string)AY_OBFUSCATE("license file not exist!"));
    }
    ...
    // aes decrypt
    std::string key_hex =
        (std::string)AY_OBFUSCATE("d37dffe9718aa504f3624518628b3156b88e8ac8552b4a892ac02cfd93b40c16");
    std::string iv_hex = (std::string)AY_OBFUSCATE("20b2af9f545582950f267dec8596d190");
    ...
    // std::cout << (std::string)AY_OBFUSCATE("Signature length: ") << signature.size() << std::endl;
    if (!rsaVerify(data, signature, public_key)) {
        throw std::runtime_error((std::string)AY_OBFUSCATE("fail to verify signature"));
    }
    ...
    if (now > std::stoll(license_endtime)) {
        throw std::runtime_error((std::string)AY_OBFUSCATE("license is expired!"));
    }

    // check license_tag
    if (license_tag != (std::string)AY_OBFUSCATE(LICENSE_TAG)) {
        throw std::runtime_error((std::string)AY_OBFUSCATE("license tag is not matched!"));
    }

    // check license_env
    if (license_env == (std::string)AY_OBFUSCATE("anymachine")) {
        std::cout << (std::string)AY_OBFUSCATE("license is valid!\n");
    } else {
        ...
        if (!found && license_env != info) {
            throw std::runtime_error((std::string)AY_OBFUSCATE("license env is invalid!"));
        }
        std::cout << (std::string)AY_OBFUSCATE("license is valid!\n");
    }
}
...
PYBIND11_MODULE(license_validator, m) {
    m.def(
        "check_license", [](const std::string &path_str) { glm::check_license(std::filesystem::path(path_str)); },
        "check_license");
}